...
WIDTH, HEIGHT = 1200, 800  # экранның размері
FPS = 90  
draw = False 
radius = 2  #ручканың размері радиусы
color = 'blue'  # ручканың түсі
mode = 'pen'  

pygame.init()
screen = pygame.display.set_mode([WIDTH, HEIGHT]) 
pygame.display.set_caption('Paint')  # экранның аты
clock = pygame.time.Clock() 
screen.fill(pygame.Color('white'))  
font = pygame.font.SysFont('None', 60)  # шрифты
...
def drawLine(screen, start, end, width, color):
    ...
    # екі координатаның разницасын есептеу
    dx = abs(x1 - x2)
    dy = abs(y1 - y2)
...
def drawCircle(screen, start, end, width, color):
    #бастапқы және соңғы точкалардағы координаталар
    x1 = start[0]
    x2 = end[0]
    y1 = start[1]
    y2 = end[1]
